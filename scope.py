...
#   fast mode:   0.37sec
    ...
    def readScope(self, channel="CH1", fast_mode=False):
        ...
        try:
            #######NEU###### TESTEN ####
            #Freeze Oszi
            self.inst.write("ACQ:STATE 0")
            #Save the required channel data in a buffer
            self.inst.write("SAV:WAVE CH1, REFA")
            self.inst.write("SAV:WAVE CH2, REFB")
            #Unfreeze Oszi
            self.inst.write("ACQ:STATE 1")
            
            
		
            # select channel
#            self.inst.write("DAT:SOU "+channel)
            self.inst.write("DAT:SOU REFA") #for testing only CH1
            # set encoding
